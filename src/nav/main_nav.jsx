...
export default function ManiNav() {
    ...
    return (
        <header
            className=" flex justify-between items-center w-screen bg-slate-400 p-1 shadow-md sticky z-50
        ">
            ...
            <div onClick={() => setServiceShow(prev => !prev)} className="absolute top-12 ">
                <span className="bg-gray-200 border p-1 rounded-sm">service</span>
                {sidebarshow && <Sidebar />}
            </div>
            <div className="  flex flex-col-reverse justify-end sm:flex-row ">
                <div className={`${menuState ? "hidden" : "block"} flex flex-col absolute z-200000 top-22 bg-gray-400 rounded-sm p-2 w-auto right-0 gap-2
                    sm:static sm:flex-row sm:flex sm:gap-8 `}
                >
